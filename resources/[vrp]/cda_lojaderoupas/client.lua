...
Tunnel.bindInterface("cda_lojaderoupas", vRPrp)
mapreedevSERVER = Tunnel.getInterface("cda_lojaderoupas")


local lojaderoupa = {
    -- LOJA DE ROUPA 01
    { name = "Loja de Roupas", id = 73, color = 4, x = -1835.8975830078, y = -1214.119140625, z = 13.017219543457, provador = {x = -1835.8975830078, y = -1214.119140625, z = 13.017219543457, heading = 359.29}},
    { name = "Loja de Roupas", x = -1834.5101318359, y = -1214.9099121094, z = 13.0172290802, provador = {x = -1834.5101318359, y = -1214.9099121094, z = 13.0172290802, heading = 356.65}},
    { name = "Loja de Roupas", x = -1832.9437255859, y = -1215.7911376953, z = 13.017255783081, provador = {x = -1832.9437255859, y = -1215.7911376953, z = 13.017255783081, heading = 3.41}},
...
function SetCameraCoords()
	...
	if not DoesCamExist(cam) then
        ...
        --TaskPlayAnim(ped, "mp_sleep", "bind_pose_180", 8.0, 0.0, -1, 9, 0, 0, 0, 0 )
        pos = GetEntityCoords(PlayerPedId())
        camPos = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 0.0, 0.0)
        SetCamCoord(cam, camPos.x, camPos.y+0.45, camPos.z+0.65)
...
RegisterNUICallback("changeCam", function(data, cb)
    if data.part == "cam1" then
        pos = GetEntityCoords(PlayerPedId())
        camPos = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 0.0, 0.0)
        SetCamCoord(cam, camPos.x, camPos.y+0.45, camPos.z+0.65)
        PointCamAtCoord(cam, pos.x, pos.y, pos.z+0.65)
    elseif data.part == "cam2" then
        pos = GetEntityCoords(PlayerPedId())
        camPos = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 0.0, 0.0)
        SetCamCoord(cam, camPos.x, camPos.y+0.95, camPos.z+0.55)
        PointCamAtCoord(cam, pos.x, pos.y, pos.z+0.55)
    elseif data.part == "cam3" then
        pos = GetEntityCoords(PlayerPedId())
        camPos = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 0.0, 0.0)
        SetCamCoord(cam, camPos.x, camPos.y+0.55, camPos.z-0.90)
        PointCamAtCoord(cam, pos.x, pos.y, pos.z-0.90)
    elseif data.part == "cam4" then
        pos = GetEntityCoords(PlayerPedId())
        camPos = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 0.0, 0.0)
        SetCamCoord(cam, camPos.x, camPos.y+2.00, camPos.z+0.20)
        PointCamAtCoord(cam, pos.x, pos.y, pos.z+0.20)
    end
end)

RegisterNUICallback("changePart2", function(data, cb)
    dataPart = parts[data.part]
    local ped = PlayerPedId()
    if GetEntityModel(ped) == GetHashKey("mp_m_freemode_01") then
        SendNUIMessage({ 
            changeCategory2 = true, 
            sexo = "Male", prefix = "M", 
            drawa = vRP.getDrawables(dataPart), category = dataPart,
        })
    elseif GetEntityModel(ped) == GetHashKey("mp_f_freemode_01") then 
...
CreateThread(function()
    while true do
        ...
        for k, v in pairs(lojaderoupa) do
            ...
            if GetDistanceBetweenCoords(playerCoords.x, playerCoords.y, playerCoords.z, lojaderoupa[k].x, lojaderoupa[k].y, lojaderoupa[k].z, true ) <= 5 and not in_loja and (cooldown == 0) then
                sleep = 4
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(ped), lojaderoupa[k].x, lojaderoupa[k].y, lojaderoupa[k].z, true ) < 1 then
                ...
                if IsControlJustPressed(0, 38) --[[ and emP.Procurado() ]]  then    
                    -- if not vPROCURADO.checkprocurado() then 
...
RegisterNUICallback("scrollHeading", function(data, cb)
    heading = data.value
    SetEntityHeading(PlayerPedId(), heading)
end)
...
function updateCarroCompras()
    ...
    for k, v in pairs(carroCompras) do
        if carroCompras[k] == true then
            valor = valor + 300 -- VALOR DE CADA ITEM
        end
    end
    precoTotal = valor
    return valor
...
RegisterNUICallback("payament", function(data, cb)
    ...
    noProvador = false
    chegou = false
    if mapreedevSERVER.shopBuy(tonumber(data.price)) then
    else
